...
class ShaderProgram {
  ...
  constructor(canvas: HTMLCanvasElement, vertexShaderSrc: string, fragmentShaderSrc: string) {
    this.canvas = canvas
    this.vertexShaderSrc = vertexShaderSrc
    this.fragmentShaderSrc = fragmentShaderSrc
    this.devicePixelRatio = Math.min(window.devicePixelRatio, 2)

    const { renderingCtx, webGlProgram } = setupRenderingCtx(canvas, vertexShaderSrc, fragmentShaderSrc)
    ...
    canvas.addEventListener('touchmove', (e) => {
      this.updatePointerPosition(e.targetTouches[0].clientX, e.targetTouches[0].clientY)
    })
  ...
  render = () => {
    ...
    this.renderingCtx.uniform1f(this.renderingCtx.getUniformLocation(this.webGlProgram, 'u_time'), currentTime)
